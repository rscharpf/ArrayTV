gcCorrectMain <-
    function(Ms, chr, starts, samplechr, nodes, increms,
	...
	if(returnOnlyTV) {
		...
		gmaxvalsInd <- apply(tvScore, 2, which.max)
                ...
                priorFracWremaining <- getGCinBestWindowGenome(gcFracBoth,starts,nparts,chr,samplechr,remainingChr,increms,gmaxvals,gmaxvalsInd,
                tvScore,verbose=verbose)

                if(onlyGC){
                        result <- priorFracWremaining
                    }}else{
                            priorFracWremaining=providedGC
                            userProvidedGC=T;gmaxvalsInd=0;tvScore=0;gcFracBoth=0;nparts=0;samplechr=''
                            increm <- 0; increm2 <- 0;
                        }
