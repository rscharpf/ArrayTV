...
optnum=4  
 ...
 for(hh in 1:((nparts/optnum)*2)){
  ...
  if(hh==1){   
  gcFracA=(partitionedgcFrac[seq(seqinc,length(partitionedgcFrac),nparts/optnum)]+(priorFrac*priorWeight))/(priorWeight+1)
}else{
  gcFracA=(gcFracA*((priorWeight+hh-1)/(priorWeight+hh)))+partitionedgcFrac[seq(seqinc,length(partitionedgcFrac),nparts/optnum)]/(priorWeight+hh)
      }
  
  newsp=split(useM[,ar],round(gcFracA,2))
  toplot=sapply(newsp,mean)   
#  toplot=sapply(newsp,median)
