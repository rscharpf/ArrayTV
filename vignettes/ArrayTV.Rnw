...
While our analysis is limited to chromosome 15, in practice we
recommend implementing the wave correction on all autosomes
simultaneously.  Next, we specify \Robject{increms} and \Robject{wins}
to indicate the size of the genomic intervals surrounding the markers
from which GC content will be computed. Here each of the 3 entries in
the vector \Robject{increms} specifies an increment size, with the
corresponding entries in \Robject{wins} specifying the maximum window
size to evaluate for that increment. For example, the following code
specifies three series of windows to evaluate:
...
\noindent The first series of windows has maximum window size of 100
and increment value 20, indicating that windows 20, 40, \ldots, and
100 will be evaluated. For the second series, the maximum windowis
10e3 and windows 2000, 4000, \ldots, 10e3 will be evaluated. Hence,
for each series the number of windows to be evaluated is 5.  When
multiple series are specified, the same number of windows must be
evaluated.  When multiple cores are available, computation can proceed
in parallel for computing the GC content for the above windows.  To
enable parallelization across 8 cores using the multicore package, one
could execute the following (unevaluated) code chunk:
...
function uses the \Robject{increms} and \Robject{max.window} vectors to
...
par(mfrow=c(1,3), mar=c(2.5,0.05,1.6,0.05),
    mgp=c(1.5,.5,0),
    oma=c(0,4,4,0), las=1)
windowsize <- c('small','medium','large')
plotTV(list(nimcM1List,agcM1List,afcM1List),
       increms, max.window, windowsize,
